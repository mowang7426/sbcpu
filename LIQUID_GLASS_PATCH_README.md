...
- 删除 `glassTintLayer` 创建（`_glassTintLayer = nil`），同步代码自动跳过
- 文字阴影加强：黑色 65%、offset (0,1)、radius 3.0
- 新增 UILabel 自带白色外发光：`shadowColor 白 55%`、`shadowOffset (0,0)`，模拟描边
- 纯透明玻璃，背景最清晰，文字在任何背景上都突出可读

**调参**：搜 `LGApplyGlassLabelShadow`，改黑色阴影透明度/半径、白色外发光透明度；
想恢复玻璃厚度就把 init 里 `_glassTintLayer = nil` 改回 tint 层创建代码（见 V6 备份）。
