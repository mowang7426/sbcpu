...
float2 backdropSampleUV(float2 capturePx,
                        ...
                        constant Uniforms &u)
{
    // 液态修复 v2：统一局部直接采样。src 纹理即本区域内容（backdrop 捕获），
    // 折射位移直接作用在局部 UV 上。此前非锁屏走“屏幕→壁纸”映射，
    // 但映射参数未接入（cardOrigin=0、分辨率硬编码），导致采样错位/黑色。
    float2 sampleUV = (capturePx + displacementPx) / u.resolution;
...
static float        g_sbcpuFresnelGlareStrength = 0.5f;
...
static NSString *lgPrefsPath(void) {
    ...
    dispatch_once(&once, ^{
        cached = jbroot(@"/var/mobile/Library/Preferences/com.yourname.sbcpufloating.plist");
    });
    return cached;
}
static NSString * const kLGPrefsReloadNote = @"com.yourname.sbcpufloating.prefschanged";
...
static float lgClamp01(NSNumber *value, float fallback) {
...
static float lgSBCPUFloat(NSDictionary *prefs, NSString *key, float fallback) {
    return lgClamp01(prefs[[NSString stringWithFormat:@"SBCPU.LiquidGlass.%@", key]], fallback);
}
...
static void lgReloadHostPrefs(void) {
    ...
    g_sbcpuFresnelGlareStrength = lgClamp01(prefs[@"SBCPU.LiquidGlass.Specular"], 0.65f);
    ...
    for (int i = 0; i < kHostCount; i++) {
        ...
        if (!strcmp(kHostDefaults[i].prefPrefix, "SBCPUFloating")) {
            // SBCPU owns this surface: use its own controls instead of the
            // original LiquidGlassPrefs global multipliers.
            float overall = lgSBCPUFloat(prefs, @"Strength", 0.75f);
            float refraction = lgSBCPUFloat(prefs, @"Refraction", 0.65f);
            float thickness = lgSBCPUFloat(prefs, @"Thickness", 0.70f);
            float dispersion = lgSBCPUFloat(prefs, @"Dispersion", 0.20f);
            float specular = lgSBCPUFloat(prefs, @"Specular", 0.65f);
            NSNumber *bezelNumber = prefs[@"SBCPU.LiquidGlass.Bezel"];
            float bezel = [bezelNumber isKindOfClass:NSNumber.class]
                ? fminf(1.50f, fmaxf(0.40f, bezelNumber.floatValue)) : 0.90f;
            NSNumber *indexNumber = prefs[@"SBCPU.LiquidGlass.RefractiveIndex"];
            float refractiveIndex = [indexNumber isKindOfClass:NSNumber.class]
                ? fminf(2.2f, fmaxf(1.01f, indexNumber.floatValue)) : 1.70f;
            g_hostParams[i].glassThickness *= overall * thickness;
            g_hostParams[i].refractionScale *= overall * refraction;
            g_hostParams[i].dispersionStrength *= overall * dispersion;
            g_hostParams[i].tintStrength *= overall;
            g_hostParams[i].darkTintStrength *= overall;
            g_hostParams[i].bezelRatio *= bezel;
            g_hostParams[i].refractiveIndex = refractiveIndex;
            g_sbcpuFresnelGlareStrength = specular * overall;
        } else {
...
#define R13TRACE(...) do { if (callN <= 3) lglog(__VA_ARGS__); } while (0)
    ...
    lu.fresnelGlareStrength = !strcmp(hp->prefPrefix, "SBCPUFloating")
        ? g_sbcpuFresnelGlareStrength
        : g_fresnelGlareStrength;
    ...
    // 液态修复 v2：自定义玻璃完全替换系统 gaussian blur，不再垫一层系统模糊
    // （此前先跑原 blur 再叠玻璃，导致“多一层液态”且玻璃不透明）
    R13TRACE("R13[%llu] skip stock gaussian (custom glass replaces it)", callN);
    uint64_t t_afterGauss = mach_absolute_time();
    R13TRACE("R13[%llu] after skipped stock gaussian", callN);
