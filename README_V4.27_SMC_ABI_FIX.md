# SBCPU V4.27 Charge Engine — SMC ABI Fix

这份工程是在 V4.26 Stable 工程上继续修复。原有 SBCPU Floating / Thermal / Control Center / Force120 目标保留。

## 本版新增关键修复

0. **修复 AppleSMC user-client ABI：** 原 V4.26 的 `SMCParamStruct` 实际为 164 字节，`vers`/`pLimitData` 布局与 Battman 不一致；iOS 17 AppleSMC 因输入结构长度/字段布局错误返回 `0xe00002c2 (kIOReturnBadArgument)`，导致 CH0C/CH0I 所有写入失败。现改为 Battman 使用的 168 字节 arm64 布局，并加入编译期 `static_assert`。
1. `smc_write_key()` 同时检查 IOKit 传输错误和 SMC 固件 `result` 字段。
